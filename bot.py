...
def admin_menu_kb() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="💳 Платёжные реквизиты"), KeyboardButton(text="🏠 Главное меню")],
        ],
        resize_keyboard=True
    )
...
async def create_bot_name(message: Message, state: FSMContext):
    ...
    await message.answer("✅ Название принято.\n\n👮 <b>Шаг 3/6</b> — Введите Telegram ID администратора магазина:")
...
async def create_bot_yoomoney_wallet(message: Message, state: FSMContext):
    ...
    # Если кошелёк указан, запрашиваем токен
    if yoo_wallet != "-":
...
async def finalize_bot_creation(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Для управления платежами нажмите кнопку «💳 Платёжные реквизиты» в админ-панели.",
        reply_markup=main_menu_kb()
    )
...
# ── Управление платёжными реквизитами ─────────────────────

@router.message(F.text == "💳 Платёжные реквизиты")
async def payment_settings_menu(message: Message):
    """Показывает меню управления платёжными реквизитами"""
    async with async_session_maker() as session:
        result = await session.execute(
            select(ShopBot).where(ShopBot.owner_id == message.from_user.id)
        )
    ...
    if not bots:
        return await message.answer("У вас пока нет ботов. Создайте бота!")

    buttons = [(f"{bot.bot_name} (ID: {bot.id})", f"payment_settings:{bot.id}") for bot in bots]
    await message.answer("💳 Выберите бота для настройки платежей:", reply_markup=inline_kb(buttons))
...
async def edit_crypto_token_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "💎 Введите новый токен Crypto Bot (от @CryptoBot):\n"
        "Или «-» чтобы удалить.",
        reply_markup=cancel_kb()
    )
    await callback.answer()
...
async def edit_yoo_token_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "🔑 Введите новый OAuth-токен ЮMoney:\n"
        "Или «-» чтобы удалить.",
        reply_markup=cancel_kb()
    )
    await callback.answer()
...
async def test_payment_menu(message: Message):
    """Меню для тестирования оплаты"""
    async with async_session_maker() as session:
        result = await session.execute(
            select(ShopBot).where(ShopBot.owner_id == message.from_user.id)
        )
    ...
    buttons = [(f"{bot.bot_name}", f"test_payment:{bot.id}") for bot in bots]
    await message.answer("Выберите бота для тестовой оплаты:", reply_markup=inline_kb(buttons))
...
async def select_payment_method(callback: CallbackQuery):
    ...
    if not categories:
        return await callback.answer("Нет категорий. Добавьте товары.")

    # Выбираем первую категорию и первый товар для теста
    cat = categories[0]
    ...
    if not products:
        return await callback.answer("Нет товаров в категории.")
...
async def test_crypto_payment(callback: CallbackQuery):
    ...
    if not invoice:
        return await callback.answer("Ошибка создания счёта!", show_alert=True)
...
async def check_yoomoney_payment_status(callback: CallbackQuery):
    ...
    if result:
        ...
        await callback.message.answer(
            ...
            f"Тестовый платёж прошёл успешно! 🎉"
        )
    else:
