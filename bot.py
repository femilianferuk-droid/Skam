...
class Emoji:
    # ID for icon_custom_emoji_id in buttons
    ...
    ADD_ID = "5771851822897566479"
    LIST_ID = "5778479949572738874"
    REMOVE_ID = "5771851822897566479"
    TEST_ID = "5774022692642492953"
    LOLZ_ID = "5373141891321699086"
    ...
    ADD = f'<tg-emoji emoji-id="{ADD_ID}">➕</tg-emoji>'
    LIST = f'<tg-emoji emoji-id="{LIST_ID}">📋</tg-emoji>'
    REMOVE = f'<tg-emoji emoji-id="{REMOVE_ID}">➖</tg-emoji>'
    TEST = f'<tg-emoji emoji-id="{TEST_ID}">🧪</tg-emoji>'
    LOLZ = f'<tg-emoji emoji-id="{LOLZ_ID}">🎮</tg-emoji>'
    ...
    EYE = f'<tg-emoji emoji-id="{EYE_ID}">👁</tg-emoji>'
...
def is_rollypay_configured(bot: ShopBot) -> bool:
    """Проверяет, что все 3 поля RollyPay заполнены для бота"""
...
def is_lolz_admin_configured(config: AdminConfig) -> bool:
    """Проверяет, что все поля Lolzteam заполнены в админке"""
...
def payment_method_sub_kb(tier: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💰 RollyPay", callback_data=f"sub_pay:{tier}:rollypay")],
        [InlineKeyboardButton(text="🎮 Lolzteam", callback_data=f"sub_pay:{tier}:lolz")],
    ])
...
def admin_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🎮 Lolzteam", callback_data="admin_lolz")],
...
def payment_method_kb(product_id: int, bot: ShopBot) -> InlineKeyboardMarkup:
    ...
    Проверяет все поля перед показом кнопки.
    """
...
async def admin_command(message: Message):
    ...
    status_text = (
        ...
        f"🎮 Lolzteam: {'✅ Настроен' if is_lolz_admin_configured(config) else '❌ Не настроен'}\n"
...
async def sub_payment_process(callback: CallbackQuery):
    ...
    # Проверка реквизитов для выбранного метода
...
async def check_payment_status(callback: CallbackQuery):
    """Проверка статуса платежа (для подписок и покупок)"""
    parts = callback.data.split(":")
    method = parts[1]
    payment_id = parts[2]
    ...
    async with async_session_maker() as session:
        ...
        # Ищем подписку по разным критериям
...
async def create_bot_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"{Emoji.PEOPLE} <b>Шаг 3/5</b> — Введите Telegram ID администратора:\n"
...
async def my_bots(message: Message):
    ...
    for bot in bots:
        ...
        text = (
            ...
            f"▸ Покупателей: {buyers_count} | Выручка: {total_revenue or 0} ₽\n"
...
async def manage_categories(callback: CallbackQuery):
    ...
    text = f"{Emoji.FILE} <b>Категории бота «{bot.bot_name}»</b>\n\n"
    if cats:
...
async def add_category_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        f"Например: «🎁 Акции» или «🔥 Хиты продаж»",
        reply_markup=cancel_kb()
    )
    await callback.answer()
...
async def add_product_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"{Emoji.FILE} Выберите категорию для товара:",
...
async def del_product_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"{Emoji.FILE} Выберите категорию:",
...
async def payment_settings(callback: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👾 Изменить Crypto Bot", callback_data=f"edit_crypto:{bot_id}")],
        [InlineKeyboardButton(text="💳 Изменить ЮMoney", callback_data=f"edit_yoo:{bot_id}")],
        [InlineKeyboardButton(text="💰 Изменить RollyPay (3 токена)", callback_data=f"edit_rolly_bot:{bot_id}")],
        [InlineKeyboardButton(text="🎮 Изменить Lolzteam (API + Merchant)", callback_data=f"edit_lolz_bot:{bot_id}")],
...
async def edit_rolly_bot_save(message: Message, state: FSMContext):
    ...
    status_text = f"{Emoji.CHECK} RollyPay полностью настроен!" if configured else "⚠️ RollyPay сохранён, но не все поля заполнены!"
...
async def edit_lolz_bot_token(message: Message, state: FSMContext):
    ...
    await message.answer("🏪 Введите Merchant ID (или «-»):")
...
async def edit_lolz_bot_save(message: Message, state: FSMContext):
    ...
    status_text = f"{Emoji.CHECK} Lolzteam полностью настроен!" if configured else "⚠️ Lolzteam сохранён, но не все поля заполнены!"
    await message.answer(status_text, reply_markup=main_menu_kb())
...
async def bot_stats(callback: CallbackQuery):
    ...
    text = (
        f"{Emoji.STATS} <b>Статистика — «{bot.bot_name}»</b>\n\n"
...
async def bot_buyers(callback: CallbackQuery):
    ...
    text = f"{Emoji.PEOPLE} <b>Топ покупателей — «{bot.bot_name}»</b>\n\n"
    if buyers:
...
async def admin_yoomoney(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("💳 Введите кошелёк ЮMoney:", reply_markup=cancel_kb())
    await callback.answer()
...
async def admin_lolz_save(message: Message, state: FSMContext):
    ...
    async with async_session_maker() as session:
        ...
        if st == "lolz_token":
            ...
            await message.answer("🏪 Введите Merchant ID:")
            return
...
async def admin_stats(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        f"🤖 Ботов всего: {total_bots} (активных: {active_bots})\n"
...
def create_shop_router(bot_record: ShopBot) -> Router:
    ...
    async def my_purchases(message: Message):
        ...
        for purchase, product in rows:
            text += (
                f"🛍 {product.name}\n"
                f"   💰 {purchase.amount} ₽ | {sm.get(purchase.status, purchase.status)}\n"
    ...
    async def shop_profile(message: Message):
        ...
        if last_purchase:
            ...
            text += (
                f"\n🕐 Последняя покупка:\n"
                f"   {sm.get(last_purchase.status, '❓')} {last_purchase.amount} ₽ "
