...
class LolzteamAPI:
    """FIXED Lolzteam API - uses correct endpoints and token format"""
    ...
    async def create_invoice(
        ...
        try:
            ...
            logger.info(f"Lolzteam create invoice request:")
            logger.info(f"  URL: {self.BASE_URL}/invoice")
            logger.info(f"  Headers: {json.dumps({k: v[:10]+'...' if k=='Authorization' else v for k,v in headers.items()})}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    logger.info(f"Lolzteam response body: {response_text}")
                    
                    # Handle HTTP errors
                    if resp.status == 401:
                        logger.error("Lolzteam 401 Unauthorized - check API token and permissions (need 'invoice' scope)")
                        return None
                    
                    if resp.status == 403:
                        logger.error("Lolzteam 403 Forbidden - check if merchant_id is correct and token has access")
                        return None
                    
                    if resp.status == 429:
                        logger.error("Lolzteam 429 Too Many Requests - rate limit exceeded")
                        return None
                    
                    if resp.status != 200:
                        logger.error(f"Lolzteam unexpected status: {resp.status}")
                        return None
                    ...
                    try:
                        ...
                        logger.error(f"Lolzteam: failed to parse JSON response")
                        return None
                    ...
                    if not invoice_data:
                        ...
                        if "url" in data and "invoice_id" in data:
                            invoice_data = data
                        else:
                            logger.error(f"Lolzteam: no invoice data in response: {list(data.keys())}")
                            return None
                    ...
                    if not invoice_url:
                        logger.error("Lolzteam: no 'url' field in invoice data")
                        return None
                    
                    if not invoice_id:
                        logger.error("Lolzteam: no 'invoice_id' field in invoice data")
                        return None
                    
                    logger.info(f"Lolzteam invoice created successfully:")
                    logger.info(f"  invoice_id: {invoice_id}")
                    logger.info(f"  url: {invoice_url}")
                    logger.info(f"  status: {invoice_status}")
            ...
            logger.error("Lolzteam request timed out after 20 seconds")
            return None
    ...
    async def check_invoice(self, invoice_id: int) -> Optional[str]:
        try:
            ...
            logger.info(f"Lolzteam checking invoice: {invoice_id}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    response_text = await resp.text()
                    logger.info(f"Lolzteam check invoice {invoice_id} status: {resp.status}")
                    logger.info(f"Lolzteam check invoice {invoice_id} body: {response_text}")
                    
                    if resp.status != 200:
                        logger.error(f"Lolzteam check invoice {invoice_id} failed with status {resp.status}")
                        return None
                    
                    try:
                        data = json.loads(response_text)
                    except json.JSONDecodeError:
                        logger.error(f"Lolzteam check invoice {invoice_id}: failed to parse JSON")
                        return None
                    
                    if "errors" in data:
                        logger.error(f"Lolzteam check invoice {invoice_id} errors: {data['errors']}")
                        return None
            ...
            logger.error(f"Lolzteam check invoice {invoice_id} error: {e}")
            return None
...
def payment_settings_menu_kb(bot_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=f"{Emoji.CHANNEL} Инструкции: {CHANNEL_USERNAME}", url=CHANNEL_URL)],
    ...
    async with async_session_maker() as session:
        ...
        if method == "crypto":
            ...
            if result and result.get("url"):
                ...
                sub.payment_id = payment_id
                await session.commit()
                logger.info(f"Lolzteam invoice created: {invoice_id}, url: {pay_url}")
            else:
                logger.error(f"Failed to create Lolzteam invoice for sub {sub.id}")
                await callback.answer(f"{Emoji.CROSS} Ошибка создания платежа Lolzteam! Проверьте токены в {CHANNEL_USERNAME}", show_alert=True)
                return
...
async def bot_settings(callback: CallbackQuery):
    ...
    text = (
        ...
        f"📝 Приветственное сообщение: {'✅ Настроено' if bot.welcome_message else '❌ По умолчанию'}\n"
        f"📞 Поддержка: @{bot.support_username or 'не указана'}\n\n"
        f"Выберите действие:"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{Emoji.PENCIL} Приветственное сообщение", callback_data=f"edit_welcome:{bot_id}")],
...
async def edit_welcome_save(message: Message, state: FSMContext):
    ...
    async with async_session_maker() as session:
        await session.execute(
            update(ShopBot).where(ShopBot.id == bot_id).values(welcome_message=welcome)
        )
        await session.commit()
    
    await message.answer(f"{Emoji.CHECK} Приветственное сообщение обновлено!", reply_markup=main_menu_kb())
...
async def edit_support_save(message: Message, state: FSMContext):
    ...
    async with async_session_maker() as session:
        await session.execute(
            update(ShopBot).where(ShopBot.id == bot_id).values(support_username=support)
        )
        await session.commit()
...
# ── Управление товарами (расширенное) ───────────────────────
...
async def edit_deliver_save(message: Message, state: FSMContext):
    ...
    async with async_session_maker() as session:
        await session.execute(
            update(Product).where(Product.id == product_id).values(auto_deliver_text=text)
        )
        await session.commit()
...
async def list_products(callback: CallbackQuery):
    ...
    for i, (cat_name, p) in enumerate(all_products[start:end], start + 1):
        ...
        # Add manage buttons for each product
        kb.inline_keyboard.append([
            InlineKeyboardButton(
            ...
            InlineKeyboardButton(
                text=f"{'❌ Скрыть' if p.is_available else '✅ Показать'}", 
    ...
    kb.inline_keyboard.append([
        InlineKeyboardButton(text=f"{Emoji.ADD} Добавить", callback_data=f"add_product:{bot_id}"),
        InlineKeyboardButton(text=f"{Emoji.BACK} Назад", callback_data=f"back_to_bot:{bot_id}")
    ])
...
async def payment_settings(callback: CallbackQuery):
    ...
    await callback.message.edit_text(text, reply_markup=payment_settings_menu_kb(bot_id))
    await callback.answer()
...
async def admin_lolz(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(f"{Emoji.LOLZ} <b>Lolzteam: API Token</b>\nВведите API Token (нужен scope: invoice):\n\nИнструкции: {CHANNEL_USERNAME}", reply_markup=cancel_kb())
    await callback.answer()
...
# ═══════════════════════════════════════════════════════════
# SHOP BOT FACTORY (UPDATED WITH AUTO-DELIVERY AND SUPPORT)
# ═══════════════════════════════════════════════════════════
...
def create_shop_router(bot_record: ShopBot) -> Router:
        ...
        async with async_session_maker() as session:
            ...
            if method == "crypto":
                ...
                if result and result.get("url"):
                    ...
                    payment_id = str(invoice_id)
                    logger.info(f"Shop: Lolzteam invoice created: {invoice_id}")
                else:
                    logger.error(f"Shop: Failed to create Lolzteam invoice")
    ...
    async def shop_check_payment(callback: CallbackQuery):
        ...
        is_paid = False
        async with async_session_maker() as session:
            # Search for purchase by payment_id OR id
            result = None
            if payment_id.isdigit():
                result = await session.execute(
            ...
            if purchase and purchase.status == "completed":
                ...
                if invoice_id:
                    api = LolzteamAPI(bot_record.lolz_api_token, bot_record.lolz_merchant_id)
                    status = await api.check_invoice(invoice_id)
                    logger.info(f"Shop: Lolzteam invoice {invoice_id} status: {status}")
                    if status == "paid":
                        is_paid = True
    ...
    async def my_purchases(message: Message):
        async with async_session_maker() as session:
            # Fixed: proper join between Purchase and Product
            result = await session.execute(
