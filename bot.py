...
class LolzteamAPI:
    """Fixed Lolzteam API using correct endpoints from documentation"""
    ...
    async def create_invoice(
        ...
        try:
            ...
            logger.info(f"Creating Lolzteam invoice: amount={amount}, payment_id={payment_id}, merchant={self.merchant_id}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    logger.info(f"Lolzteam API response status: {resp.status}")
                    logger.info(f"Lolzteam API response body: {response_text}")
                    
                    try:
                        data = json.loads(response_text)
                    except:
                        logger.error(f"Failed to parse Lolzteam response: {response_text}")
                        return None
                    ...
                    if "errors" in data:
                        ...
                        logger.error(f"Lolzteam API error: {error_msg}")
                        return None
                    
                    # Check for invoice in response
                    if "invoice" in data:
                        ...
                        logger.info(f"Lolzteam invoice created successfully: {result['invoice_id']}")
                        return result
                    ...
                    if "url" in data:
                        ...
                        logger.info(f"Lolzteam invoice created (alt format): {result['invoice_id']}")
                        return result
                    
                    logger.error(f"Unexpected Lolzteam response format: {data}")
                    return None
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    data = await resp.json()
                    logger.info(f"Lolzteam check response: {data}")
                    ...
                    logger.error(f"Unexpected check response: {data}")
                    return None
        except Exception as e:
    ...
    async with async_session_maker() as session:
        ...
        if method == "crypto":
            ...
            if result and result.get("url"):
                ...
                await callback.answer(f"{Emoji.CROSS} Ошибка создания платежа Lolzteam! Проверьте токены.", show_alert=True)
                return
...
async def confirm_del_product_direct(callback: CallbackQuery):
    ...
    if bot_id:
        await list_products(callback, bot_id, 0)
...
# ═══════════════════════════════════════════════════════════
# SHOP BOT FACTORY (UPDATED WITH AUTO-DELIVERY)
# ═══════════════════════════════════════════════════════════
