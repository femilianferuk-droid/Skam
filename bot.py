...
class Emoji:
    """Standard Unicode emojis that work everywhere"""
...
class User(Base):
    """User model for storing user data and subscription info"""
...
class Subscription(Base):
    """Subscription model for tracking user subscription payments"""
...
class AdminConfig(Base):
    """Admin configuration for global payment settings"""
...
async def run_migrations():
    ...
    async with engine.begin() as conn:
        ...
        for col_name, col_type in bot_migrations:
        ...
        for col_name, col_type in purchase_migrations:
        ...
        for col_name, col_type in admin_migrations:
...
async def init_db():
    """Initialize database with migrations and default config"""
...
async def can_create_bot(
    ...
    """Check if user can create more bots based on their subscription"""
...
class RollyPayAPI:
    """RollyPay payment API integration (SBP payments)"""
...
def feedback_menu_kb(bot: ShopBot) -> ReplyKeyboardMarkup:
    """Menu for feedback-type child bots with optional feedback button"""
    keyboard = []
...
def bot_management_kb(
    ...
    """Management keyboard for a specific bot"""
...
def inline_kb(
    ...
    """Create inline keyboard from list of (text, callback) tuples"""
    return InlineKeyboardMarkup(
        inline_keyboard=[
...
def payment_method_kb(
    ...
    """Payment method keyboard for a specific product"""
...
def payment_invoice_kb(
    ...
    """Payment invoice with pay and check buttons"""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
...
def payment_settings_menu_kb(bot_id: int) -> InlineKeyboardMarkup:
    """Payment settings menu with channel link"""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
...
def bot_settings_kb(
    ...
    """Bot settings keyboard - different for shop and feedback"""
...
async def admin_cmd(message: Message):
    ...
    crypto_ok = "✅" if config.crypto_bot_token else "❌"
    yoo_ok = "✅" if config.yoomoney_wallet else "❌"
    rolly_ok = (
    ...
    admin_text = (
        ...
        f"{Emoji.CRYPTOBOT} Crypto Bot: {crypto_ok}\n"
        f"{Emoji.CARD} ЮMoney: {yoo_ok}\n"
        f"{Emoji.COINS} RollyPay: {rolly_ok}\n"
...
async def create_bot_start(message: Message, state: FSMContext):
    """Start bot creation - choose type first"""
...
async def create_bot_token(message: Message, state: FSMContext):
    ...
    async with async_session_maker() as session:
        ...
        if result.scalar_one_or_none():
            return await message.answer(
                f"{Emoji.CROSS} Бот с таким токеном уже существует."
            )
...
async def create_bot_finish(message: Message, state: FSMContext):
    """Finish shop bot creation with payment setup"""
    if message.text == f"{Emoji.CROSS} Отмена":
        await state.clear()
        return await message.answer(
            "Отменено.", reply_markup=main_menu_kb()
    ...
    yoomoney = None if yoo == "-" else yoo
    
    await state.update_data(yoomoney_wallet=yoomoney)
    await finish_bot_creation(message, state)
...
async def edit_feedback_button_save(
    ...
    """Save feedback button text and ask for reply"""
    if message.text == f"{Emoji.CROSS} Отмена":
        await state.clear()
        return await message.answer(
            "Отменено.", reply_markup=main_menu_kb()
...
async def reply_feedback_send(
    ...
    """Send reply to feedback message VIA the child bot"""
    if message.text == f"{Emoji.CROSS} Отмена":
        await state.clear()
        return await message.answer(
            "Отменено.", reply_markup=main_menu_kb()
    ...
    async with async_session_maker() as session:
        ...
        # Отправляем ответ через дочерний бот
        if bot_token:
            try:
                child_bot = Bot(
                    token=bot_token,
                ...
                await message.answer(
                    ...
                    f"дочерний бот: {e}\n\n"
                    f"Попробуйте ответить напрямую: "
                    f"/reply_{msg_id} [ваш текст]"
                )
        else:
            await message.answer(
...
async def payment_settings(callback: CallbackQuery):
    """Show payment settings for a shop bot"""
    bot_id = int(callback.data.split(":")[1])
    ...
    crypto_ok = "✅" if bot.crypto_bot_token else "❌"
    yoo_ok = "✅" if bot.yoomoney_wallet else "❌"
    rolly_ok = (
        "✅" if is_rollypay_configured(bot) else "❌"
    )
...
async def test_payment(callback: CallbackQuery):
    """Create test payment for a shop bot"""
    bot_id = int(callback.data.split(":")[1])
...
def create_shop_router(bot_record: ShopBot) -> Router:
    ...
    async def shop_start(message: Message):
        ...
        await message.answer(
            welcome, reply_markup=shop_menu_kb()
        )
...
def create_feedback_router(bot_record: ShopBot) -> Router:
    ...
    async def fb_start(message: Message):
        ...
        welcome = bot_record.welcome_message or (
            ...
            f"Отправьте ваше сообщение, и администратор ответит вам."
        )
    ...
    async def fb_support(message: Message):
        ...
        if bot_record.support_username:
            await message.answer(
                f"{Emoji.PHONE} <b>Поддержка</b>\n\n"
                f"По вопросам: @{bot_record.support_username}",
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
    ...
    async def handle_feedback_message(message: Message):
        """Handle any message as feedback - EXCLUDING reply commands"""
        # Ignore menu buttons
        if message.text in [
        ...
        # Ignore /reply_ commands (these are handled by constructor bot)
        if message.text and message.text.startswith("/reply_"):
            return
        
        # Also ignore if it's the feedback button text
        if (
            bot_record.feedback_button_text
            and message.text == bot_record.feedback_button_text
        ):
            # This is handled as regular feedback below
            pass
        ...
        async with async_session_maker() as session:
            ...
            try:
                admin_text = (
                    f"{Emoji.MAIL} <b>Новое сообщение #{fb_msg.id}</b>\n"
                ...
                admin_text += (
                    ...
                    f"\n\n📌 Бот: {bot_record.bot_name} (ID: {bot_record.id})"
                ...
                logger.info(
                    f"Feedback message #{fb_msg.id} forwarded to admin "
        ...
        # Send reply to user
        reply_text = bot_record.feedback_button_reply or (
            f"{Emoji.CHECK} Ваше сообщение #{fb_msg.id} "
            f"отправлено! Администратор ответит в ближайшее время."
        )
...
async def run_shop_bot(bot_record: ShopBot):
    """Start a child bot (shop or feedback)"""
    ...
    async def polling():
        logger.info(
            ...
            f"(id={bot_record.id}, type={bot_record.bot_type}) started"
        )
        try:
...
async def start_all_active_bots():
    """Start all active bots on launch"""
    async with async_session_maker() as session:
        result = await session.execute(
    ...
    logger.info(f"Starting {len(bots)} active bots...")
