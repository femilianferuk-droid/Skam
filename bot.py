...
Runs constructor bot + all active shop bots.
"""
...
class CryptoBotAPI:
    """Crypto Bot API (https://pay.crypt.bot/api)"""
...
class YooMoneyAPI:
    """YooMoney API — только генерация формы оплаты по номеру кошелька."""

    def __init__(self, wallet: str):
        self.wallet = wallet

    def generate_form_url(self, amount: float, label: str, comment: str) -> str:
        """Создать URL для формы оплаты ЮMoney."""
        return (
...
async def create_bot_crypto(message: Message, state: FSMContext):
    ...
    await message.answer(
        "💸 <b>Шаг 5/5</b> — Введите номер кошелька ЮMoney (например: 410011234567890).\n"
        "Или отправьте <b>-</b> чтобы пропустить."
    )
...
async def create_bot_finish(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Перейдите в созданного бота и нажмите /start",
        reply_markup=main_menu_kb()
    )
...
async def payment_settings(callback: CallbackQuery):
    ...
    text = (
        ...
        f"💸 ЮMoney кошелёк: {'✅ Настроен' if bot.yoomoney_wallet else '❌ Не настроен'}\n\n"
        f"Выберите действие:"
    )
    ...
    if bot.crypto_bot_token:
        kb.inline_keyboard.append([
            InlineKeyboardButton(
                ...
                callback_data=f"do_crypto:{bot_id}:{product.id}"
            )
        ])
    if bot.yoomoney_wallet:
        kb.inline_keyboard.append([
            InlineKeyboardButton(
                ...
                callback_data=f"do_yoo:{bot_id}:{product.id}"
            )
        ])
...
def create_shop_router(bot_record: ShopBot) -> Router:
    """Создаёт роутер для дочернего магазина."""
    shop_router = Router()
    ...
    async def buy_donate(message: Message):
        ...
        buttons = [(cat.name, f"shop_cat:{cat.id}") for cat in categories]
    ...
    async def show_products(callback: CallbackQuery):
        ...
        buttons = [(f"{p.name} — {p.price} ₽", f"shop_product:{p.id}") for p in products]
        await callback.message.answer(
    ...
    async def product_detail(callback: CallbackQuery):
        ...
        if bot_record.yoomoney_wallet:
            payment_buttons.append(("💸 ЮMoney", f"pay_yoo:{product_id}"))
        ...
        await callback.message.answer(
            ...
            reply_markup=inline_kb(payment_buttons)
        )
        await callback.answer()
    ...
    async def admin_payment_settings(message: Message):
        ...
        text = (
            f"💳 <b>Платёжные реквизиты бота «{bot_record.bot_name}»</b>\n\n"
            f"💎 Crypto Bot: {'✅ Настроен' if bot_record.crypto_bot_token else '❌ Не настроен'}\n"
            f"💸 ЮMoney кошелёк: {'✅ Настроен' if bot_record.yoomoney_wallet else '❌ Не настроен'}\n\n"
            f"Выберите действие:"
        )

        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💎 Изменить Crypto Bot токен", callback_data=f"edit_crypto:{bot_record.id}")],
            [InlineKeyboardButton(text="💸 Изменить ЮMoney кошелёк", callback_data=f"edit_yoo:{bot_record.id}")],
        ])
