...
def feedback_menu_kb(bot: ShopBot) -> ReplyKeyboardMarkup:
    """Menu for feedback-type child bots"""
    keyboard = []
...
def bot_settings_kb(
    ...
    # Feedback bots have additional button settings
...
async def reply_feedback_send(
    ...
    """Send reply to feedback message"""
    if message.text == f"{Emoji.CROSS} Отмена":
        await state.clear()
        return await message.answer(
            "Отменено.", reply_markup=main_menu_kb()
    ...
    async with async_session_maker() as session:
        ...
        try:
            ...
            await message.answer(
                f"{Emoji.CROSS} Ошибка отправки: {e}"
            )
...
async def get_bot_name(bot_id: int) -> str:
    """Helper to get bot name by ID"""
    async with async_session_maker() as session:
        bot = await session.get(ShopBot, bot_id)
        return bot.bot_name if bot else "Неизвестный"
...
def create_feedback_router(bot_record: ShopBot) -> Router:
    ...
    async def fb_start(message: Message):
        ...
        await message.answer(
            welcome,
            reply_markup=feedback_menu_kb(bot_record)
        )
    ...
    async def handle_feedback_message(message: Message):
        """Handle any message as feedback"""
        # Skip menu buttons
        if message.text in [
...
async def run_shop_bot(bot_record: ShopBot):
    ...
    if bot_record.bot_type == "shop":
        ...
        dp.include_router(
            create_feedback_router(bot_record)
        )
